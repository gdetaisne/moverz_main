...
export function generateCorridorPageMetadata(destination: string): Metadata {
  return {
    title: `Déménagement {{{city_name}}} → ${destination} — Comparez des devis | Moverz`,
    description: `Un seul dossier, 20 déménageurs qualifiés. 3 devis sous 7 jours pour {{{city_name}}} → ${destination}.`,
    alternates: {
      canonical: `https://www.{{{domain}}}/{{{citySlug}}}-vers-${destination.toLowerCase()}`,
    },
    openGraph: {
      title: `Déménagement {{{city_name}}} → ${destination} — Comparez des devis | Moverz`,
      description: `Un seul dossier, 20 déménageurs qualifiés. 3 devis sous 7 jours pour {{{city_name}}} → ${destination}.`,
      url: `https://www.{{{domain}}}/{{{citySlug}}}-vers-${destination.toLowerCase()}`,
      type: 'website',
    },
  };
}
...
export function generateCorridorPageJsonLd(destination: string) {
  return {
    ...
    "@type": "Service",
    "name": `Déménagement {{{city_name}}} → ${destination} — comparaison de devis`,
    "provider": {
      "@type": "Organization",
      "name": "Moverz"
    },
    "areaServed": `{{{city_name}}} → ${destination}`,
      ...
      <div className="min-h-screen bg-[#04163a]">
          ...
          <div className="container max-w-7xl mx-auto px-4 md:px-6 py-12">
            <div className="text-center">
              <h1 className="text-3xl md:text-5xl font-bold text-white mb-6">
                Déménagement {{{city_name}}} → {destination} : comparez des devis fiables
              </h1>
        ...
        <div className="container max-w-6xl mx-auto px-4 md:px-6 py-12">
          <h2 className="text-2xl font-bold text-white mb-8 text-center">
            Prix indicatifs {{{city_name}}} → {destination}
          </h2>
          <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="container max-w-4xl mx-auto px-4 md:px-6 py-12">
          <div className="bg-white/5 backdrop-blur rounded-2xl p-8 border border-white/10 text-center">
            <h2 className="text-xl font-semibold text-white mb-4">
              Vous déménagez depuis {{{city_name}}} ?
            </h2>
