...
export function getCityDataFromUrl(siteUrl: string): CityData {
  // Extraire la ville depuis l'URL (ex: devis-demenageur-marseille.fr)
  const match = siteUrl.match(/demenageur[s]?[-_]([a-z]+)/i);
  const citySlug = match ? match[1] : 'toulouse';
  
  // Cas spéciaux
  if (siteUrl.includes('toulousain')) return cityData.toulouse;
  if (siteUrl.includes('bordeaux-demenageur')) return cityData.bordeaux;
